      ...
      <div className="animate-pulse space-y-6">
        <div className="rounded-3xl backdrop-blur-xl border border-white/30 bg-white/20 dark:bg-gray-800/20 px-4 py-4 sm:px-6 sm:py-6 shadow-lg dark:border-white/10">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="h-10 w-full sm:w-72 rounded-2xl bg-white/50 dark:bg-gray-700/50" />
        ...
        <div className="grid gap-6 md:gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            ...
            >
              <div className="h-40 rounded-2xl bg-white/60 dark:bg-gray-700/50" />
              <div className="h-6 w-3/4 rounded bg-white/70 dark:bg-gray-700/60" />
              <div className="h-4 w-5/6 rounded bg-white/60 dark:bg-gray-700/50" />
              <div className="h-4 w-1/2 rounded bg-white/60 dark:bg-gray-700/50" />
              <div className="flex flex-wrap gap-2">
                {Array.from({ length: 3 }).map((_, tagIdx) => (
                  <div
                    key={tagIdx}
                    className="h-6 w-16 rounded-full bg-white/70 dark:bg-gray-700/60 border border-white/30 dark:border-white/10"
                  />
                ))}
              </div>
            </div>
