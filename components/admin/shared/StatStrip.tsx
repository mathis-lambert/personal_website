...
import type { InsightMetric } from "@/types/analytics";
...
export function MetricDelta({
  ...
  previous,
  compact = false,
}: InsightMetric & { compact?: boolean }) {
  if (previous === 0) {
    return (
      <span className="t-meta text-ink-faint">
        {value > 0 ? (compact ? "new" : "first in this range") : compact ? "—" : "nothing yet"}
      </span>
    );
  }
      ...
      {stats.map((stat) => (
        <div key={stat.label} data-ink={stat.ink} className="bg-paper-lift p-4">
          ...
          <dd>
            ...
            <div className="mt-2">
              <MetricDelta {...stat.metric} />
            </div>
            <MetricSparkline values={stat.series} />
          </dd>
        </div>
      ))}
    </dl>
