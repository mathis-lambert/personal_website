...
const FloatingChatInput: React.FC<ChatInputProps> = ({ placeholder = 'Poser une question' }) => {
    ...
    const adjustTextAreaHeight = useCallback(() => {
        ...
        if (textArea) {
            ...
            textArea.style.lineHeight = textArea.value.trim() === '' ? `${minHeight}px` : 'normal';
        }
    }, [minHeight, maxHeight]);
            ...
            >
                <form onSubmit={handleSubmit} className="relative flex items-end w-full">
                    ...
                    <Button
                        type="submit"
                        size="icon"
                        className="absolute bottom-0 right-0 w-8 h-8 bg-sky-500/50 shadow-lg backdrop-blur-lg rounded-lg flex items-center justify-center hover:bg-sky-500/60 text-xs disabled:opacity-50 disabled:cursor-not-allowed dark:bg-sky-600/50 dark:hover:bg-sky-600/60 transition-colors duration-200 ease-in-out"
                        disabled={isLoading || !message.trim()}
                        aria-label="Envoyer le message"
                    >
                        {isLoading ? <Loader2 size={16} className="text-white animate-spin" /> : <ArrowUp size={16} className="text-white" />}
                    </Button>
                </form>
            </motion.div>
