...
const ResumePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setErr(null);
      try {
        const data = await getCollectionData<Record<string, unknown>>(
          'resume',
          token,
        );
  ...
  const save = async () => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) throw new Error('JSON must be an object');
      await replaceCollection('resume', parsed, token);
      alert('Saved');
    } catch (e) {
